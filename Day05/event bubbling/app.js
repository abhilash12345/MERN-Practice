...
for (let i = 0; i < recipes.length; i++) {
  ...
  newContainer.className = "card1";
  newContainer.innerHTML = `    
        <h3>${recipes[i].name} </h3>
        <img src = "${recipes[i].image} "/>
        <${recipes[i].cuisine} /p>
        <p>${recipes[i].tags.join(", ")} </p> 
        <button onClick = "addrecipeToCart(event)">Start</button>
                                         `;

  parentElem.appendChild(newContainer);
}

const addrecipeToCart = (e) => {
  recipeCount++;
  console.log("Recipe Added", recipeCount);
};

//-----------------------------------------------

const handleDummyParentCardClick = (e) => {
  console.log("parent clicked", e);
};

const handleDummyButtonCardClick = (e) => {
  console.log("button clicked", e);
};

const dummyRootClicked = () => {
  console.log("root clicked");
};

const dummyBodyClicked = () => {
